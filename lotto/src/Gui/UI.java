...
public class UI extends JFrame implements ActionListener {
	...
	Method method = new Method(var, DAO);
	...
	// UI 클래스 생성자
	public UI(variable var, DAO DAO, Method method) {
		this.var = var;
		this.DAO = DAO;
		this.method = method;
		...
		// 숫자 이미지 라벨
		ImageIcon im = new ImageIcon("src/numbers_png/title.png");
...
//		var.rank.setToolTipText("드롭박스 기록을 상세하게 보여줍니다.");
		...
		for (i = 0; i < var.showBall.length; i++) {
			for (int j = 0; j < var.showBall[i].length; j++) {
				...
				var.showBall[i][j].setBounds(45 * j + 70, 20 + i * 60, 50, 30);
